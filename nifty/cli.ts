...
async function main() {
  ...
  const answer_mint_quantity = await askQuestion("How many you want?");
  const answer_mint_id = await askQuestion("What Id should it be?");
  await nft_collection.mint({
    receivingAddress: answer_mint_address,
    quantity: parseInt(answer_mint_quantity),
    idsToMint: [parseInt(answer_mint_id)],
  });
  */
